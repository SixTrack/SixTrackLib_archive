...
class Sequence(object):
	...
	def add_kick(self,kv,order):
		...
		newfid=len(self.dataf)
		self.lst.extend([newid])
		self.datai.extend([newid,3,newfid,None,order])
		...
		for index, value in enumerate(self.datai): 
			if value==None:
				if self.lst[i]==None:
					...
					i = i+1
		return self
	def track(self,np,nturn,seq):
		self.join()
		c_np = ctypes.c_int(np)
		...
		val = libc.vec_map_loop(arg_dataf,arg_datai,arg_coord,c_np,c_nturn,c_seq)
