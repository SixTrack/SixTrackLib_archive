...
inline void thin4d_exact_drift_calc(INT elem_floatid, FLOAT x, FLOAT y, FLOAT px, FLOAT py, FLOAT PathLengthDiff, FLOAT PhysicalLengthOfBlock, FLOAT RatioBetaToBetaj, FLOAT partf[]){
    ...
    SETCOORDF(partf,PathLengthDiff,PathLengthDiff*OnePoweredTo3);
}

INT thin6d_exact_drift_map(INT elemi[], FLOAT elemf[], INT elemid, INT parti[], FLOAT partf[], INT partid, INT partn){
    INT cntexactdrift;
    ELEMINIT;
    INITPARTF;
    GETCOORDF(partf,x);
    ...
    GETCOORDF(partf,px);
    GETCOORDF(partf,py);
    GETCOORDF(partf,PathLengthDiff);  //doubt! (no such variable in partf)
    GETCOORDF(partf,RatioBetaToBetaj);  //doubt! (no such variable in partf)

    GETATTRF(thin6d_exact_drift,PhysicalLengthOfBlock);

    thin4d_exact_drift_calc(elem_floatid, x, y, px, py, PathLengthDiff, PhysicalLengthOfBlock, RatioBetaToBetaj, GETPARTF(partid));
